export default {
  home: {
    eyebrow: 'Awense',
    heading: 'Where would you like to start?',
    subtitle: 'Two services. One platform.',
    greeting: 'Welcome back, ',
    travel: {
      label: 'Travel Planning',
      desc: 'Plan day-by-day itineraries, share with companions, and explore destinations.',
      cta: 'Explore →',
    },
    ai: {
      label: 'AI Marketplace',
      desc: 'Access Claude, GPT-4o, Gemini and more on a single pay-per-use wallet.',
      cta: 'Explore →',
    },
  },
  nav: {
    login: 'Login',
    profile: 'Profile',
    home: 'Home',
    logout: 'Logout',
    travel: 'Travel',
    iotDevice: 'IoT Device',
    health: 'Health',
    budget: 'Budget',
    applePay: 'Apple Pay',
    ssKey: 'Siri Shortcuts',
  },
  landing: {
    eyebrow: 'Plan. Explore. Connect.',
    title: 'Your all-in-one\nadventure workbench.',
    subtitle:
      'Build travel itineraries, access AI models, and track everyday goals — all in one place. No account needed to start.',
    startPlanning: 'Start Planning →',
    signIn: 'Sign In',
    badge1: 'Free to start',
    badge2: 'No account needed',
    highlights: {
      h1: 'Travel planner — free, no account required',
      h2: 'AI marketplace — Claude, GPT-4o, Gemini and more',
    },
    trust: {
      travel: '✈ Travel Planner',
      ai: '🤖 AI Marketplace',
    },
    features: {
      eyebrow: 'Your toolkit',
      sectionTitle: 'Travel planning meets frontier AI.',
      planning: {
        title: 'Travel Planning',
        desc: 'Build day-by-day itineraries, add activities and accommodation, and share with travel companions.',
      },
      ai: {
        title: 'AI Marketplace',
        desc: 'One account. Multiple frontier models. Access Claude, GPT-4o, and Gemini on a single pay-per-use wallet.',
      },
      tools: {
        title: 'Lifestyle Tools',
        desc: 'HDB housing lookup, IPPT fitness tracker, Douyin live checker — practical tools built for Singapore.',
      },
    },
    cta: {
      heading: 'Start building your workbench.',
      sub: 'Free travel planner. No account needed to begin.',
      action: 'Start Planning',
      secondary: 'Create Account',
    },
    footerNote: 'Save and sync your itineraries by {link}.',
    signingIn: 'signing in',
    auth: {
      greeting: 'Hi, ',
      walletBalance: 'Wallet balance',
      topUp: 'Top Up →',
      actions: {
        marketplace: 'AI Marketplace',
        marketplaceDesc: 'Chat with leading LLMs',
        wallet: 'Top Up Wallet',
        walletDesc: 'Add credits to your account',
        apiKey: 'API Key',
        apiKeyDesc: 'Your personal API key',
      },
    },
  },
  footer: {
    tagline: 'Your all-in-one workbench',
    allRights: 'All rights reserved',
    betaBadge: 'Beta',
    sections: {
      modules: 'Modules',
      personal: 'Personal',
      plan: 'Plan',
      general: 'General',
    },
    links: {
      home: 'Home',
      profile: 'Profile',
      workbench: 'Workbench',
      aiMarketplace: 'AI Marketplace',
    },
  },
  workbench: {
    eyebrow: 'Awense',
    title: 'Your Workbench',
    subtitle: 'Select a module to get started',
    tiles: {
      pphs:        { label: 'PPHS',             desc: 'Public housing info, bus stops and MRT proximity' },
      flat:        { label: 'Flat Analysis',     desc: 'Search by postal code · MRT and bus stop proximity' },
      scenic:      { label: 'Scenic Spots',      desc: 'China 5A scenic spots · Track your visits by province' },
      travel:      { label: 'Travel',            desc: 'Itinerary planning and collaboration' },
      douyin:      { label: 'Douyin Live',       desc: 'Check if a Douyin user is livestreaming' },
      telegram:    { label: 'Telegram Storage',  desc: 'Upload media via Telegram bot and retrieve by ID' },
      imghost:     { label: 'Image CDN',         desc: 'Upload an image · Get a shareable CDN link' },
      ippt:        { label: 'IPPT · Strider',    desc: 'Track push-ups, sit-ups and 2.4km run · Project your score' },
      marketplace: { label: 'Marketplace',       desc: 'Browse and chat with AI-powered services' },
      bus:         { label: 'Bus Timings',       desc: 'Find your nearest bus stop and live arrival times' },
      admin:       { label: 'Admin',             desc: 'Manage users, roles, and system settings' },
    },
  },
  bus: {
    eyebrow: 'Live Transit',
    title: 'Nearby Bus Timings',
    subtitle: 'Find your nearest bus stop and see real-time arrival times.',
    locating: 'Finding your location…',
    locateButton: 'Use My Location',
    retry: 'Retry',
    errors: {
      unsupported: 'Geolocation is not supported on this device.',
      permissionDenied: 'Location access was denied. Enable it in your browser settings and try again.',
      locationFailed: 'Could not determine your location. Please try again.',
      noStopsFound: 'No bus stops found near your location.',
      busstopFetchFailed: 'Failed to fetch nearby bus stops.',
      arrivalsFetchFailed: 'Failed to fetch bus arrival timings.',
    },
    nearbyStops: 'Nearby Stops',
    nearestBadge: 'Nearest',
    distanceAway: '{distance} away',
    noServices: 'No live bus service data for this stop.',
    arriving: 'Arr',
    wheelchairAccessible: 'Wheelchair accessible bus',
    minutesShort: '{n} min',
    updated: 'Updated {time}',
    refreshNow: 'Refresh',
    load: {
      SEA: 'Seats Available',
      SDA: 'Standing Available',
      LSD: 'Limited Standing',
    },
    busType: {
      DD: 'Double-decker bus',
      BD: 'Bendy bus',
    },
  },
  iot: {
    key: {
      title: 'IoT Device Key',
      subtitle: 'Generate an API key so your ESP32 (or other device) can authenticate with the /iot endpoint.',
      cardTitle: 'API Key Management',
      statusActive: 'Active',
      statusNotConfigured: 'Not configured',
      deviceNameLabel: 'Device name',
      deviceNamePlaceholder: 'e.g. hike-hitcher ESP32',
      generate: 'Generate API Key',
      regenerate: 'Regenerate',
      revoke: 'Revoke',
      createdOn: 'Created {date}',
      keyHintLabel: 'Key identifier',
      freshWarning: 'Copy your API key now — it will not be shown again. You must regenerate to get a new one.',
      copy: 'Copy',
      dismiss: "I've saved it — dismiss",
      noKeyDesc: 'Generate an API key to let your device authenticate with the /iot endpoint.',
      regenerateConfirmMessage: 'Regenerating will revoke your current API key. Continue?',
      regenerateConfirmTitle: 'Regenerate Key',
      revokeConfirmMessage: 'This will permanently revoke your API key. Your device will stop being able to authenticate until you generate a new one.',
      revokeConfirmTitle: 'Revoke Key',
      confirm: 'Confirm',
      cancel: 'Cancel',
      usageTitle: 'Using with your device',
      usageStep1: 'Store the key in your device firmware (e.g. secrets.h)',
      usageStep2: 'Send it as the x-api-key header on every request',
      usageEndpointsTitle: 'Endpoints',
      usagePostBodyLabel: 'Body:',
      usageGetQueryLabel: 'Query:',
    },
  },
  ssKey: {
    title: 'Siri Shortcuts API Key',
    subtitle: 'Generate an API key so Siri Shortcuts automations can authenticate with your account.',
    cardTitle: 'API Key Management',
    statusActive: 'Active',
    statusNotConfigured: 'Not configured',
    generate: 'Generate API Key',
    regenerate: 'Regenerate',
    revoke: 'Revoke',
    createdOn: 'Created {date}',
    keyHintLabel: 'Key identifier',
    freshWarning: 'Copy your API key now — it will not be shown again. You must regenerate to get a new one.',
    copy: 'Copy',
    dismiss: "I've saved it — dismiss",
    noKeyDesc: 'Generate an API key to let Siri Shortcuts automations authenticate with your account.',
    regenerateConfirmMessage: 'Regenerating will revoke your current API key. Continue?',
    regenerateConfirmTitle: 'Regenerate Key',
    revokeConfirmMessage: 'This will permanently revoke your API key. Any Shortcuts using it will stop working until you generate a new one.',
    revokeConfirmTitle: 'Revoke Key',
    cancel: 'Cancel',
    usageTitle: 'Using with Siri Shortcuts',
    usageStep1: 'Open the Shortcuts app and open (or create) your shortcut',
    usageStep2: 'Add a Get Contents of URL action, set Method to POST and Request Body to JSON',
    usageStep3: 'Under Headers, add: X-API-Key → paste your key',
    usageStep4: 'Set the URL to the endpoint below',
    usageEndpointsTitle: 'Endpoints',
    usageBodyLabel: 'Body:',
  },
  travel: {
    landing: {
      eyebrow: 'Travel',
      title: 'Every journey begins with a single step.',
      subtitle: 'Build day-by-day itineraries, collaborate with travel companions, and share your trips.',
      startPlanning: 'Start Planning →',
      myTrips: 'My Trips →',
      newDraft: 'New Draft →',
      signIn: 'Sign In',
      heroImageAlt: 'A traveler in flowing robes walking a misty mountain ridge, ink-wash painting',
      cta: {
        heading: 'Ready to start planning?',
        sub: 'Free to use. No account needed to create a draft.',
        action: 'Start Planning',
        secondary: 'Create Account',
      },
      footerNote: 'Save and sync your itineraries by {link}.',
      signingIn: 'signing in',
    },
    list: {
      title: 'My Trips',
      subtitle: 'Plan, organise, and share your travel itineraries',
      newTrip: '+ New Trip',
      myTrips: 'My Trips',
      noTrips: 'No trips yet. Create your first one!',
      sharedWithMe: 'Shared with Me',
      noShared: 'No shared trips yet.',
      datesTbc: 'Dates TBC',
      destinationTbc: 'Destination TBC',
      pax: '{n} pax',
      viewTrip: 'View trip',
      copyLink: 'Copy share link',
      deleteTrip: 'Delete trip',
      linkCopied: 'Share link copied!',
      deleteConfirm: 'Delete "{title}"? This cannot be undone.',
      deleteTitle: 'Delete Trip',
      delete: 'Delete',
      cancel: 'Cancel',
      createTrip: 'Create Trip',
      loginPrompt: 'Please log in to view your trips.',
      login: 'Log In',
      failed: 'Failed to create trip. Please try again.',
      noTitle: 'Please enter a trip title.',
      deleted: 'Trip deleted.',
      failedDelete: 'Failed to delete trip.',
      untitledTrip: 'Untitled Trip',
      startDraft: 'Start planning — no account needed',
      startDraftSub: 'Your trip saves locally until you sign in',
      dialog: {
        title: 'New Trip',
        tripTitle: 'Trip Title',
        tripTitlePlaceholder: 'e.g. Tokyo Spring 2026',
        destination: 'Destination',
        destinationPlaceholder: 'e.g. Tokyo, Japan',
        travelDates: 'Travel Dates',
        to: 'to',
        startDate: 'Start date',
        endDate: 'End date',
        numberOfTravellers: 'Number of Travellers',
      },
    },
    planner: {
      loginPrompt: 'Please log in to edit this trip.',
      login: 'Log In',
      back: '← Trips',
      save: 'Save',
      exportBtn: 'Export ↓',
      exportJson: 'Export as JSON',
      exportCsv: 'Export as CSV',
      exportIcs: 'Export Apple Calendar (.ics)',
      share: 'Share 🔗',
      accessCodeSet: 'Access code set',
      noAccessCode: 'No access code',
      tripTitle: 'Trip title',
      destination: 'Destination',
      start: 'Start',
      end: 'End',
      pax: 'Pax',
      paxPlaceholder: 'e.g. Dad, Mum, Me',
      noItems: 'No agenda items yet.',
      noSchedule: 'Nothing scheduled yet — assign a day to a picked item to see it here.',
      addFirstItem: '+ Add First Item',
      addToDay: '+ Add to Day {n}',
      addToDateTbc: '+ Add to Date TBC',
      addItem: '+ Add Item',
      unsaved: 'Unsaved',
      saved: 'Saved',
      editTooltip: 'Edit',
      deleteTooltip: 'Delete',
      bookings: 'Bookings',
      booked: '{n} / {total} booked',
      noBookings: 'No bookings yet. Click "+ Add Booking" to get started.',
      form: '📋 Form',
      map: '🗺️ Map',
      tripSaved: 'Trip saved.',
      loginToSave: 'Please log in to save.',
      saveFailed: 'Failed to save. Please try again.',
      saveFirst: 'Save the trip first to generate a share link.',
      notYours: "This itinerary doesn't belong to you. Please create a new trip.",
      accessCodeApplied: 'Access code set. Save your trip to apply.',
      noAccessCodeSet: 'No access code set — trip remains public.',
      preview: 'Preview',
      autoSaving: 'Saving…',
      autoSaved: '✓ Saved',
      autoSaveFail: '⚠ Save failed',
      draftBanner: 'Your trip is saved locally. Sign in to sync and share.',
      draftMigrated: 'Your draft has been saved to your account!',
      schedule: 'Your Schedule',
    },
    viewer: {
      protected: 'Protected Itinerary',
      protectedDesc: 'This itinerary is protected. Enter the 6-digit access code to view it.',
      wrongCode: 'Incorrect code. Please try again.',
      unlock: 'Unlock',
      notFound: 'Trip not found',
      notFoundDesc: 'This itinerary link may have expired or been removed.',
      goToTrips: 'Go to My Trips',
      traveller: 'traveller',
      travellers: 'travellers',
      view: 'view',
      views: 'views',
      map: 'Trip Map',
      noItems: 'No agenda items yet.',
      expandAll: 'Expand All',
      collapseAll: 'Collapse All',
      item: 'item',
      items: 'items',
      untitled: 'Untitled',
      bookings: 'Bookings',
      exportBtn: 'Export ↓',
      exportJson: 'Export JSON',
      exportCsv: 'Export CSV',
      exportIcs: 'Export Apple Calendar (.ics)',
      booked: 'Booked',
      pending: 'Pending',
      nights: 'N',
      tableItem: 'Item',
      tableRemarks: 'Remarks',
      tablePayment: 'Payment',
      tableTotal: 'Total',
      tableDates: 'Dates',
      tableBooked: 'Booked',
      tableCancellation: 'Cancellation',
      tableBfast: 'Bfast',
    },
    agenda: {
      newItem: 'New Agenda Item',
      editItem: 'Edit Item',
      category: 'Category',
      title: 'Title',
      titlePlaceholder: 'e.g. Lunch at local restaurant',
      date: 'Date',
      selectDate: 'Select date',
      place: 'Place',
      searchPlace: 'Search for a place, landmark, city…',
      time: 'Time',
      start: 'Start',
      end: 'End',
      timeTbc: 'Time TBC',
      notes: 'Notes',
      notesPlaceholder: 'Any notes or details...',
      budget: 'Budget',
      images: 'Images',
      imagesHint: 'Images only · max {n} · {size} MB each',
      onlyImages: 'Only image files are allowed',
      fileTooLarge: '{name} exceeds {size} MB',
      maxImages: 'Maximum {n} images per agenda item',
      cancel: 'Cancel',
      addItem: 'Add Item',
      saveChanges: 'Save Changes',
      titleRequired: 'Please enter a title.',
    },
    booking: {
      addBooking: 'Add Booking',
      editBooking: 'Edit Booking',
      category: 'Category',
      flight: 'Flight',
      hotel: 'Hotel',
      transport: 'Transport',
      other: 'Other',
      item: 'Item',
      itemPlaceholder: 'e.g. Flights, Hotel (CD), Train (成都→乐山)',
      remarks: 'Remarks',
      remarksPlaceholder: 'e.g. CD, CQ, near station',
      bookingLink: 'Booking Link',
      perPersonCost: 'Per-Person Cost (SGD)',
      totalPrice: 'Total Price (SGD)',
      autoCalc: 'Auto-calculated from per-person',
      paymentTiming: 'Payment Timing',
      selectPayment: 'Select...',
      instant: 'Instant',
      delayed: 'Delayed',
      paymentDatePlaceholder: 'e.g. Apr 15, Before 18:00 Apr 7',
      checkin: 'Check-in',
      checkout: 'Check-out',
      datePlaceholder: 'YYYY-MM-DD',
      nights: 'Nights',
      autoNights: 'Auto-calculated',
      booked: 'Booked',
      freeCancellation: 'Free Cancellation',
      cancellationPlaceholder: 'e.g. Before 18:00, Apr 7',
      breakfast: 'Breakfast Included',
      deposit: 'Deposit',
      depositPlaceholder: 'e.g. 500 CNY',
      cancel: 'Cancel',
      saveChanges: 'Save Changes',
    },
    category: {
      flight: 'Flight',
      hotel: 'Hotel',
      dining: 'Dining',
      attraction: 'Attraction',
      transport: 'Transport',
      shopping: 'Shopping',
      entertainment: 'Entertainment',
      nature: 'Nature',
      other: 'Other',
    },
    suggestion: {
      activityLabel: 'Suggested for {destination}',
      placeLabel: 'Places near {destination}',
      packingLabel: 'Suggested items',
      noteLabel: 'Before you go',
      addDestination: 'Add a destination above to get activity suggestions',
      packing: {
        items: {
          umbrella:           'Umbrella',
          sunscreen:          'Sunscreen SPF 50',
          walkingShoes:       'Comfortable walking shoes',
          breathableClothing: 'Light breathable clothing',
          sunglasses:         'Sunglasses',
          mosquitoRepellent:  'Mosquito repellent',
          portableCharger:    'Portable charger',
          travelAdaptor:      'Travel adaptor',
          camera:             'Camera',
          waterBottle:        'Reusable water bottle',
          toteBag:            'Collapsible tote bag',
          handSanitiser:      'Hand sanitiser',
          wetWipes:           'Wet wipes',
          passport:           'Passport / travel docs',
          emergencyCash:      'Emergency cash',
        },
      },
    },
    packing: {
      title: 'Packing List',
      progress: '{packed} of {total} packed',
      noItems: 'No items yet. Add things to pack.',
      addItem: '+ Add Item',
      newItem: 'New Packing Item',
      editItem: 'Edit Item',
      category: 'Category',
      label: 'Item',
      labelPlaceholder: 'e.g. Passport, Sunscreen…',
      quantity: 'Quantity (optional)',
      packed: 'Mark as packed',
      saveChanges: 'Save Changes',
      cancel: 'Cancel',
      categories: {
        clothing: 'Clothing',
        toiletries: 'Toiletries',
        documents: 'Documents',
        health: 'Health',
        electronics: 'Electronics',
        misc: 'Misc',
      },
    },
    export: {
      day: 'Day',
      date: 'Date',
      title: 'Title',
      category: 'Category',
      startTime: 'Start Time',
      endTime: 'End Time',
      city: 'City',
      budget: 'Budget',
      description: 'Description',
    },
    todo: {
      title: 'Things to do',
      noItems: 'Nothing picked yet — add a suggestion above, or add your own.',
      addItem: '+ Add Item',
    },
    places: {
      title: 'Places to visit',
      noItems: 'Nothing picked yet — add a suggestion above.',
      noneFound: 'No places found nearby.',
      lookupFailed: "Couldn't load places right now — try again shortly.",
    },
    note: {
      title: 'Things to note',
      noItems: 'No notes yet.',
      addItem: '+ Add Item',
      addPromptLabel: 'What do you want to note down?',
      mandatoryBadge: 'Mandatory',
      optionalBadge: 'Optional',
      deadlineWithinDays: 'Submit within {days} days of arrival',
      deadlineBeforeLanding: 'Complete any time up to {hours}h before landing',
      openOfficialSite: 'Open official site',
    },
    createTrip: {
      dialogTitle: 'Create a trip',
      destinationLabel: 'Where are you going?',
      destinationPlaceholder: 'Search for a city or place…',
      datesLabel: 'When?',
      exactDates: 'Exact dates',
      numberOfDays: 'Number of days',
      cancel: 'Cancel',
      submit: "Let's go",
    },
    dayAssign: {
      assignToDay: 'Assign to a day',
      tooltip: 'Assign to a day',
    },
    recommendation: {
      exploreHint: 'Explore the map — click a pin to add it to your trip.',
      fillDestinationHint: 'Fill in Destination for more accurate suggestions.',
      locating: 'Locating suggestions on the map…',
      addToTrip: '+ Add to trip',
      filterLabel: 'Markers',
      filterAll: 'All',
      filterSelectedOnly: 'Selected only',
      close: 'Close',
      curatedBadge: 'Curated',
      noDetails: 'No additional details yet.',
    },
    mapStatus: {
      resolving: 'Resolving locations…',
      noDestination: 'Fill in Destination above to see suggestions here.',
      noLocationData: 'No agenda items with location data yet.',
      noFilterMatches: 'No items match the current filters.',
    },
    nav: {
      expand: 'Expand menu',
      collapse: 'Collapse menu',
    },
  },
  profile: {
    title: 'My Profile',
    subtitle: 'Manage your personal information and account security',

    avatar: {
      change: 'Change photo',
      hint: 'Click photo to upload',
      updated: 'Profile photo updated.',
      uploadFailed: 'Failed to upload photo.',
    },

    username: {
      label: 'Username',
      edit: 'Edit',
      save: 'Save',
      cancel: 'Cancel',
      placeholder: 'Display name',
      updated: 'Username updated.',
      tooShort: 'Username must be at least 3 characters.',
      taken: 'This username is already taken.',
      failed: 'Failed to update username.',
    },

    country: {
      label: 'Country',
      select: 'Select country',
      update: 'Update',
      updated: 'Country updated.',
      failed: 'Failed to update country.',
    },

    password: {
      title: 'Change Password',
      desc: 'Verify your current password before setting a new one.',
      current: 'Current Password',
      new: 'New Password',
      confirm: 'Confirm Password',
      verify: 'Verify',
      verified: 'Verified',
      incorrect: 'Incorrect password.',
      verifyFailed: 'Could not verify password.',
      mismatch: 'Passwords do not match.',
      updated: 'Password changed.',
      failed: 'Failed to change password.',
      requirement: 'Min. 12 chars, upper, number, special',
    },

    logout: {
      title: 'Danger Zone',
      button: 'Log out',
      confirm: 'Are you sure you want to log out?',
      cancel: 'Cancel',
    },
  },
  auth: {
    stepTitles: {
      email: 'Login / Register',
      register: 'Create account',
      login: 'Welcome back',
      verify: 'Verify your email',
    },

    eyebrow: {
      email: 'Sign in or create account',
      register: 'New account',
      login: 'Welcome back',
      verify: 'Verify your email',
    },

    emailChipChange: 'Change',
    forgotPassword: 'Forgot password?',
    forgotPasswordMessage: 'Password reset is not yet available. Please contact support.',

    email: {
      placeholder: 'Email address',
    },

    username: {
      placeholder: 'Display name',
    },

    password: {
      placeholder: 'Password',
    },

    verify: {
      hint: 'A 6-digit code was sent to',
      resend: "Didn't receive it?",
      resendAction: 'Resend code',
    },

    back: {
      useDifferentEmail: '← Use a different email',
    },

    buttons: {
      continue: 'Continue',
      checking: 'Checking...',
      create: 'Create account',
      creating: 'Creating account...',
      login: 'Log in',
      loggingIn: 'Logging in...',
      verify: 'Verify',
      verifying: 'Verifying...',
    },

    terms: {
      email: 'By continuing, you agree to our',
      register: 'By registering, you agree to our',
      login: 'By logging in, you agree to our',
      suffix: 'Terms and Conditions.',
    },

    errors: {
      network: 'No internet connection. Please check your network.',
      invalid_login_credentials: 'Incorrect email or password.',
      email_not_verified: 'Please verify your email before logging in.',
      email_already_registered: 'This email is already registered.',
      username_already_taken: 'This username is already taken.',
      weak_password: 'Password is too weak. Use at least 8 characters with a mix of letters and numbers.',
      registration_failed: 'Registration failed. Please try again.',
      code_expired: "Your code has expired. Click 'Resend code' to get a new one.",
      max_verify_attempts: "Too many incorrect attempts. Click 'Resend code' for a new code.",
      invalid_request_email: 'Please enter a valid email address.',
      invalid_request_password: 'Password does not meet the requirements.',
      invalid_request_username: 'Username is not valid.',
      invalid_request_generic: 'One or more fields are invalid. Please review your input.',
      unknown: 'Something went wrong. Please try again.',
      login_failed: 'Login failed. Please try again.',
      verify_failed: 'Verification failed. Please try again.',
      resend_failed: 'Could not resend the code. Please try again.',
      terms_required: 'You must agree to the terms and conditions.',
      invalid_email_format: 'Please enter a valid email address.',
      invalid_otp_format: 'Please enter the 6-digit code from your email.',
    },

    validation: {
      password_required: 'Please input password',
      password_min: 'Password must be at least 8 characters',
      identity_required: 'Please input username or email',
      identity_min: 'Username must be at least 3 characters',
      email_required: 'Please input email address',
      email_invalid: 'Please input correct email address',
      confirm_password_required: 'Please confirm password',
      confirm_password_mismatch: 'Passwords do not match',
      username_required: 'Please input username',
      username_min: 'Username must be at least 3 characters',
      terms_required: 'You must agree with the terms and conditions!',
    },

    success: {
      login: 'Login successful',
      register: 'Account created! Check your email for the verification code.',
      verified: 'Email verified! Welcome.',
      resent: 'A new code has been sent to your email.',
      logged_out: 'Logged out successfully.',
    },
  },

  marketplace: {
    public: {
      notice: 'Sign in to chat with models and manage your wallet.',
      signIn: 'Sign In',
    },
    hero: {
      eyebrow: 'Awense Marketplace',
      beta: 'Beta',
      heading: 'Your gateway to frontier AI.',
      sub1: 'One API. Multiple providers. Pay-per-use.',
      sub2: "Access the world's most capable language models with a single integration.",
      stats: {
        models: 'Models',
        providers: 'Providers',
        maxContext: 'Max Context',
        from: 'From / MTok',
      },
    },
    filter: {
      all: 'All',
      count: '{n} models',
    },
    card: {
      context: 'Context',
      maxOutput: 'Max output',
      input: 'Input',
      output: 'Output',
      perMtok: '/ MTok',
      tryChat: 'Try in Chat',
      badges: { popular: 'Popular', bestValue: 'Best Value', fastest: 'Fastest', new: 'New' },
    },
    chat: {
      empty: 'Send a message to preview this model.',
      placeholder: 'Message...',
      send: 'Send',
      hint: '↵ send',
      clear: 'Clear',
      clearConfirm: 'Clear chat with {name}?',
      cancel: 'Cancel',
    },
    chatPage: {
      back: '← Marketplace',
      clear: 'Clear',
      clearConfirm: 'Clear this conversation?',
      cancel: 'Cancel',
      lockedTooltip: 'Return to Marketplace to preview this model',
      empty: 'Send a message to start the conversation.',
      placeholder: 'Message...',
      send: 'Send',
      hint: '↵ send',
    },
    models: {
      'claude-sonnet-4-6': { description: 'The recommended model for most tasks. Excellent at coding, analysis and nuanced reasoning with a strong balance of speed and capability.' },
      'claude-opus-4-8':   { description: "Anthropic's most intelligent model. Handles the most complex tasks with superior reasoning, nuanced understanding and extended thinking." },
      'claude-haiku-4-5':  { description: "Anthropic's fastest model. Built for near-instant responses, lightweight classification, and high-throughput production workloads." },
      'gpt-4o':            { description: "OpenAI's flagship multimodal model. Strong across text, vision and audio understanding with broad general intelligence." },
      'gpt-4o-mini':       { description: 'Small and affordable. Designed for fast, focused tasks — extraction, classification, summarisation and structured outputs.' },
      'gemini-2-flash':    { description: "Google's next-generation multimodal model. Exceptional speed with native audio, image and video understanding built in." },
      'gemini-1-5-pro':    { description: "Google's long-context specialist. Handles up to 2 million tokens — ideal for entire codebases, books and large datasets." },
      'llama-3-3-70b':     { description: "Meta's open-source powerhouse. Competitive performance with full transparency — run via API or on your own infrastructure." },
    },
    back: '← Marketplace',
    accountBar: {
      balance: 'Balance',
      topUp: 'Top Up',
      apiKey: 'API Key',
    },
    wallet: {
      title: 'Wallet',
      balance: 'Available Balance',
      topUp: 'Top Up',
      customPlaceholder: 'Custom amount (e.g. 30)',
      payCard: 'Card',
      payPaynow: 'PayNow',
      mockNotice: 'Payment processing is mocked — no real charge will be made.',
    },
    apiKey: {
      title: 'API Key',
      cardTitle: 'Your API Key',
      reveal: 'Reveal',
      hide: 'Hide',
      rotateTitle: 'Rotate Key',
      rotateDesc: 'Generate a new API key. Your current key will be immediately invalidated.',
      rotateBtn: 'Rotate API Key',
      rotateConfirmTitle: 'Rotate API Key',
      rotateConfirmBody: 'Rotating your API key will immediately invalidate the current one. Any integrations using the old key will stop working.',
      rotateConfirmBtn: 'Rotate',
      usageTitle: 'How to use',
      usageDesc: 'Your API key authenticates requests to the Awense Marketplace. Keep it secret and rotate it immediately if compromised.',
      usageNote: 'External API access is coming soon.',
      copy: 'Copy',
    },
  },

  common: {
    comingSoon: 'Page coming soon.',
    cancel: 'Cancel',
  },

  toast: {
    logoutSuccess: 'Logged out successfully.',
    sessionExpired: 'Your session has expired. Please log in again.',

    notLoggedIn: 'You are not logged in. Please log in.',
    itineraryAccessDenied: 'You are not allowed to edit this itinerary. Please contact the owner.',
    loginRequired: 'You must be logged in.',
    accessDenied: 'Access denied.',

    scenicLoadFailed: 'Failed to load scenic spots.',
    scenicSaveFailed: 'Failed to save. Please try again.',

    pphsUpdateSuccess: 'PPHS record updated successfully.',
    pphsUpdateFailed: 'Failed to update PPHS record.',

    imghostImagesOnly: 'Only image files are supported.',
    imghostUploadFailed: 'Upload failed. Please try again.',
    imghostUrlCopied: 'URL copied!',
    imghostEmbedCopied: 'Embed snippet copied!',

    apiKeyCopied: 'API key copied to clipboard.',
    apiKeyCopyFailed: 'Copy failed — please copy manually.',
    apiKeyRotated: 'API key rotated. Save your new key.',
    apiKeyRotateFailed: 'Failed to rotate API key.',

    walletSelectAmount: 'Please select or enter an amount.',
    walletTopupSuccess: 'Topped up {amount} successfully.',
    walletTopupFailed: 'Top-up failed. Please try again.',

    chatCopied: 'Copied.',
    chatCopyFailed: 'Copy failed.',
    chatSessionDeleted: 'Session deleted.',
    chatFulfilled: 'Request fulfilled in {ms}ms',

    adminPricingLoadFailed: 'Failed to load pricing config.',
    adminPricingSettingsSaved: 'Global settings saved.',
    adminPricingSettingsFailed: 'Failed to save settings.',
    adminPricingModelUpdated: '{name} updated.',
    adminPricingModelFailed: 'Failed to save. Please try again.',
    adminSuggestionSaved: 'Saved.',
    adminSuggestionFailed: 'Failed to save. Please try again.',
    adminTgUserAdded: 'User {userId} added.',
    adminTgAddFailed: 'Failed to add admin.',
    adminTgRemoved: 'Admin removed.',
    adminTgRemoveFailed: 'Failed to remove admin.',
    adminUsersLoadFailed: 'Failed to load users.',
    adminRolesUpdated: 'Roles updated.',
    adminRolesFailed: 'Failed to update roles.',

    tgCopied: 'Copied: {text}',
    tgTokenFailed: 'Failed to generate token.',
    tgUploadSuccess: 'Uploaded — file sent to your Telegram DM.',
    tgBotNotStarted: 'Open a private chat with the bot and send /start first, then retry.',
    tgLinkFirst: 'Link your Telegram account first.',
    tgUploadFailed: 'Upload failed.',
    tgLoadFailed: 'Failed to load media.',
    tgNotFound: 'File not found or access denied.',
    tgDownloadFailed: 'Failed to download.',
    tgInvalidExpiry: 'Enter a valid number of days (0 = never).',
    tgExpiryUpdated: 'Expiry updated.',
    tgExpiryFailed: 'Failed to update expiry.',
    tgDeleteSuccess: 'Deleted {id}.',
    tgDeleteFailed: 'Failed to delete.',

    ssKeyCopied: 'API key copied.',
    ssKeyWarning: 'Save this key now — it will not be shown again.',
    ssKeyFailed: 'Failed to generate API key.',
    ssKeyRevoked: 'API key revoked.',

    iotKeyCopied: 'API key copied.',
    iotKeyWarning: 'Save this key now — it will not be shown again.',
    iotKeyFailed: 'Failed to generate API key.',
    iotKeyRevoked: 'API key revoked.',
    iotKeyDeviceNameRequired: 'Please enter a device name.',

    travelInvalidCode: 'Access code must be exactly 6 digits.',
    itineraryDaysDiff: 'There is a difference of {days} days. Any existing activities may need to be rescheduled.',

    healthLoadFailed: 'Failed to load health data.',
  },
  health: {
    title: 'Health',
    subtitle: 'Sleep and stress, pulled from Garmin Connect.',
    highStressBanner: 'Sustained high stress detected today:',
    stat: {
      sleepScore: 'Last Night’s Sleep Score',
      restingHr: 'Resting Heart Rate',
      currentBodyBattery: 'Body Battery',
      currentStress: 'Current Stress',
    },
    chart: {
      todayTitle: 'Today',
      trendTitle: 'Sleep & Stress Trend',
      sleepScore: 'Sleep Score',
      avgStress: 'Avg Stress',
      stress: 'Stress',
      bodyBattery: 'Body Battery',
      heartRate: 'Heart Rate',
      all: 'All',
      noDataToday: 'No data recorded yet today.',
      noDataTrend: 'No history yet — check back after a day or two.',
    },
  },
  budget: {
    template: {
      homeReno: 'Home Renovation',
      wedding: 'Wedding',
      travel: 'Travel',
      other: 'Other',
    },
    list: {
      eyebrow: 'Budget Tracker',
      title: 'Budget Tables',
      subtitle: 'Plan what to buy, track what you spent.',
      newTable: 'New Table',
      newTableTitle: 'New budget table',
      tableName: 'Table name',
      tableNamePlaceholder: 'e.g. Home Setup',
      mainCategory: 'Main category',
      mainCategoryHint: 'Sets the suggested categories for items in this table.',
      create: 'Create',
      cancel: 'Cancel',
      myTables: 'My Tables',
      sharedWithMe: 'Shared with Me',
      noTables: 'No budget tables yet.',
      noShared: 'Nothing shared with you yet.',
      itemCount: '{n} items',
      budget: 'Budget',
      actual: 'Actual',
      deleteTable: 'Delete table',
      deleteTitle: 'Delete budget table',
      deleteConfirm: 'Delete "{name}"? This cannot be undone.',
      delete: 'Delete',
      deleted: 'Table deleted',
      failed: 'Failed to create budget table',
      failedDelete: 'Failed to delete budget table',
    },
    table: {
      share: 'Share',
      shareTitle: 'Share this budget table',
      invitePlaceholder: 'Collaborator email',
      invite: 'Invite',
      inviteSuccess: 'Collaborator added',
      inviteFailed: 'Failed to add collaborator',
      remove: 'Remove',
      removeCollaboratorFailed: 'Failed to remove collaborator',
      noCollaborators: 'No collaborators yet.',
      renameFailed: 'Failed to rename table',
      notFound: 'Budget table not found.',
      noItems: 'No items yet — add one below.',
      uncategorized: 'Uncategorized',
      tab: {
        category: 'Category',
        to_buy: 'To Buy',
        bought: 'Bought',
        all: 'All',
      },
      col: {
        name: 'Name',
        category: 'Category',
        budget: 'Budget',
        actual: 'Actual',
        notes: 'Notes',
        status: 'Status',
      },
      status: {
        toBuy: 'To Buy',
        bought: 'Bought',
      },
      newName: 'New item name',
      newCategory: 'Category',
      add: 'Add',
      addFailed: 'Failed to add item',
      saveFailed: 'Failed to save changes',
      deleteFailed: 'Failed to delete item',
      itemDeletedToast: '"{name}" deleted',
      undo: 'Undo',
      restoreSuccess: 'Item restored',
      restoreFailed: 'Failed to restore item',
      statusChanged: '"{name}" moved to {status}',
      sumBudget: 'Sum Budget',
      sumActual: 'Sum Actual',
    },
  },
  dashboard: {
    eyebrow: 'Dashboard',
    title: 'Welcome back',
    subtitle: "This is your dashboard — there's nothing here yet, but there will be.",
    recentTransactions: {
      title: 'Recent Transactions',
      empty: 'No transactions yet.',
      viewAll: 'View all →',
    },
    adminPanel: {
      title: 'Admin',
      desc: 'Users, roles, and system settings.',
      open: 'Open →',
    },
  },
  applepay: {
    eyebrow: 'Apple Pay',
    title: 'Transactions',
    subtitle: 'Logged automatically from the "When Apple Pay is used" Shortcut.',
    total: 'Total',
    filteredTotal: 'Filtered Total',
    allCategories: 'All categories',
    transactionCount: '{n} transactions',
    noTransactions: 'No transactions yet.',
    setCategory: 'Set category',
    saveFailed: 'Failed to save category',
    col: {
      date: 'Date',
      merchant: 'Merchant',
      name: 'Name',
      amount: 'Amount',
      category: 'Category',
    },
  },
  admin: {
    suggestions: {
      title: 'Trip Recommendations',
      addPlace: '+ Add Place',
      addActivity: '+ Add Activity',
      destination: 'Destination Tag',
      destinationPlaceholder: 'e.g. Singapore',
      titleCol: 'Title',
      categoryCol: 'Category',
      descriptionCol: 'Description',
      actionsCol: 'Actions',
      estimatedHours: 'Estimated Hours',
      imagesLabel: 'Images',
      addImage: '+ Add Image URL',
      imagesHint: 'Paste image URLs — e.g. host one via the Image CDN (/imghost) first, then paste its link here.',
      addTitle: 'Add Recommendation',
      editTitle: 'Edit Recommendation',
      save: 'Save',
      deleteTitle: 'Delete Recommendation',
      deleteConfirm: 'Delete "{title}"? This cannot be undone.',
      deleteConfirmBtn: 'Delete',
      deleted: 'Deleted.',
    },
    logSearcher: {
      title: 'Log Searcher',
      subtitle: 'Look up a request by its Request ID and see its full server-side log tree.',
      placeholder: 'Paste a Request ID (e.g. req_a1b2c)…',
      search: 'Search',
      notFound: 'No log entries found for that Request ID.',
      invalidFormat: 'That doesn\'t look like a valid Request ID.',
      resultCount: '{n} matching log entries',
      cardTile: 'Log Searcher',
      cardDesc: 'Search server request logs by Request ID and inspect the full event tree.',
    },
    telegramLogSubscriptions: {
      title: 'Telegram Log Alerts',
      subtitle: 'Choose which backend modules send their request logs to each subscribed Telegram chat. Errors always alert regardless of this setting.',
      loadFailed: 'Failed to load Telegram log subscriptions.',
      toggleFailed: 'Failed to update — please try again.',
      chatCol: 'Chat',
      noChats: 'No Telegram chat has subscribed yet — DM the bot /start to appear here.',
      cardTile: 'Telegram Log Alerts',
      cardDesc: 'Turn Telegram log alerts on or off per chat, per backend module.',
    },
  },
}
